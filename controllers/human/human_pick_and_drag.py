...
class HumanPickAndDrag(Agent):
    
    def __init__(self, config):
        super().__init__(config)
        self.name = "human-pixel-pick-and-drag"
    ...
    def single_act(self, state):
        ...
        def mouse_callback(event, x, y, flags, param):
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                cv2.imshow('Click Pick and Drag Points (3 clicks needed)', img)
        
        cv2.imshow('Click Pick and Drag Points (3 clicks needed)', img)
        cv2.setMouseCallback('Click Pick and Drag Points (3 clicks needed)', mouse_callback)
