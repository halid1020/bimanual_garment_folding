...
class SpatialValueNet(nn.Module):
    def __init__(self, input_channel_types=None,
        ...
        channel_ids_dict = { 
            'rgb': (0, 1, 2),
            'rgb_pos': (0, 1, 2, 4, 5), ## with positional encoding
...
class MaximumValuePolicy(nn.Module, Policy):
    ...
    def get_action_single(self, state, explore=True):
        ...
        with torch.no_grad():
            value_maps = self.value_net(obs, 
                use_random_value=use_random_value)
        ...
        for primitive in self.action_primitives:
            ...
            primitive_retval[primitive]['chosen_index'] = max_index
            primitive_retval[primitive]['chosen_value'] = max_value
            primitive_retval[primitive]['all_value_maps'] = vmap
            primitive_retval[primitive]['chosen_deformable_value'] = (value_maps['deformable'][primitive].squeeze(1))[max_index]
            primitive_retval[primitive]['chosen_rigid_value'] = (value_maps['rigid'][primitive].squeeze(1))[max_index]

        valid_primitives = [primitive for primitive in self.action_primitives if primitive_retval[primitive]['chosen_index'] is not None]
        ...
        if use_random_value:
            for primitive in valid_primitives:
                ...
                primitive_retval[primitive]['chosen_index'] = real_index
                primitive_retval[primitive]['chosen_value'] = chosen_vmap[random_index]

        chosen_val = (-torch.inf, None, None)
        for primitive in self.action_primitives:
            if primitive_retval[primitive]['chosen_value'] > chosen_val[0]:
                chosen_val = (primitive_retval[primitive]['chosen_value'], primitive, primitive_retval[primitive]['chosen_index'])
        
        primitive_retval['chosen_value'] = chosen_val[0]
        primitive_retval['chosen_primitive'] = chosen_val[1]
        primitive_retval['chosen_index'] = chosen_val[2]
        primitive_retval['chosen_deformable_value'] = primitive_retval[chosen_val[1]]['chosen_deformable_value']
        primitive_retval['chosen_rigid_value'] = primitive_retval[chosen_val[1]]['chosen_rigid_value']
