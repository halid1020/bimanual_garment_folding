...
from .vanilla_image_sac import NatureCNNEncoder  # reuse the encoder
...
class ImagePrimitiveSAC(VanillaSAC):
    """
    ...
    def _make_actor_critic(self, cfg):
        ...
        self.actor = Actor(self.network_action_dim, self.feature_aug_dim, cfg.hidden_dim).to(self.device)

        self.critic = Critic(self.network_action_dim, self.feature_aug_dim).to(self.device)
       

        self.critic_target = Critic(self.network_action_dim, self.feature_aug_dim).to(self.device)
    ...
    def _update_networks(self, batch: dict):
        ...
        aug_state_taken = self._augment_emb_with_code(e, prim_idx_taken)
        ...
        aug_states_all, prim_idxs_all = self._expand_emb_all_primitives(e)
