...
class RandomShootingUVPickandPlacePlanner():
    ...
    def __init__(self, num_pick, delta_y, pull_step, wait_step,
                    ...
                    image_size=None, normalize_info=None, delta_y_range=None, task='flatten',
                    use_pred_rwd=False):
        """
    ...
    def project_3d(self, pos, matrix_world_to_camera):
        return project_to_image(matrix_world_to_camera, pos, self.image_size[0], self.image_size[1])
    ...
    def get_action(self, init_data, robot_exp=False, cloth_mask=None, check_mask=None, m_name='vsbl'):
        ...
        #matrix_world_to_camera = init_data['matrix_world_to_camera']
        ...
        select_idx = np.random.choice(idxes, self.num_pick, replace=len(idxes) < self.num_pick)
        pickup_pos = pickup_pos[select_idx]
        place_pos = place_pos[select_idx]
        idxes = idxes[select_idx]

        print('shape idex', idxes.shape)
        ...
        start_poses = pickup_pos.copy()
        after_poses = place_pos.copy()
        # else:  # simulation planning
        #     us, vs = self.project_3d(pointcloud, matrix_world_to_camera)
        #     params = [
