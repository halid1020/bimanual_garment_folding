...
\paragraph{Pick-and-Fling:} Primarily utilised for highly crumpled states or garment reorientation, pick-and-fling has proven substantially important for effective flattening. The action easily flips the garment's orientation, reveals hidden key-points, and helps untangle the cloth when needed. It is parameterised by $4$ values---one 2-D grasp point per arm.

\paragraph{Dual-arm pick-and-place:} Resembling \textit{pick-and-stretch} in ClothMate \cite{luo2025clothmate} and the \textit{fold} primitive in prior literature \cite{xue2023unifolding}, dual-arm pick-and-place adjusts the garment after initial flattening. The primitive is essential for precise folding. To prevent self-collisions, intersecting arm trajectories are executed sequentially. It is parameterised by $8$ values---a 2-D pick and a 2-D place point for each of the two arms.

\paragraph{Single-arm pick-and-place:} Also known as the \textit{drag} primitive, single-arm pick-and-place employs one robot arm to bring the garment toward the intersection of the dual-arm workspace, enabling subsequent flattening and folding. The operation is also vital for fine-grained adjustments, such as aligning trouser legs that fall outside the shared reachable area. It is parameterised by $4$ values---a single 2-D pick and 2-D place point.

\paragraph{No Operation:} Under this primitive, the robot arms remain stationary, allowing the system to stabilise upon successfully completing the task. It carries no continuous parameters ($0$-dimensional).
...
\begin{figure*}[t]
\centering
\subfloat[The architecture of MAGPIE. We snap the resulting action to the corresponding workspace, then snap the pick action onto the cloth masks before environment execution.]{%
    \includegraphics[width=0.98\linewidth]{plots/Magpie-architecture.jpg}%
    \label{fig:architecture}}\\[4pt]
\subfloat[Data sampling and augmentation. The augmentation includes rotation, flipping, scaling and colour jittering, RGB channel permutation as well as gripper-swapping. The hindsight sampling randomly selects the target states as well as the actual target of the trajectory.]{%
    \includegraphics[width=1.0\linewidth]{plots/dataloader_augment_master_multi_batch.png}%
    \label{fig:data_augmentation}}\\[4pt]
\subfloat[Interface used to collect human demonstration data and for human baseline evaluation (initial state, left; a successful fold, right).]{%
    \includegraphics[width=0.49\linewidth]{plots/human-interface-initial.png}\hfill
    \includegraphics[width=0.49\linewidth]{plots/human-interface-success.png}%
    \label{fig:human_interface}}
\caption{Overview of MAGPIE: (a) the multi-primitive network architecture, (b) the data sampling and augmentation pipeline, and (c) the interface used to collect human demonstrations and to run the human baseline.}
\label{fig:overview}
\end{figure*}
...
\begin{itemize}
    ...
    \item \textbf{Human:} A human operator folds each garment through the same primitive interface used for demonstration collection (Figure~\ref{fig:human_interface}), providing an upper-bound reference on achievable folding quality under identical observation and action constraints.
\end{itemize}
