...
\newcommand{\magpie}{\raisebox{-0.2em}{\includegraphics[height=1.2em]{magpie.pdf}}}
...
% > * **Input Block:** Show a crumpled garment RGB image and a goal folded RGB image ($96 \times 96$) passing through a Shared Vision Encoder (ResNet-18 + GroupNorm).
% > * **Latent Space:** Represent the output passing through an MLP Projector to form the 256-dimensional latent visual embedding $z$.
...
% > * **Branch 2 (Action Parameter Inference):** Create a routing switch controlled by $\omega^*$. The embedding $z$ is routed *only* to the specific 1D U-Net COT-FM network (e.g., $v_\phi^{\omega^*}$). Show a noise vector $a_0$ iteratively transported over $k=10$ integration steps into the final continuous action parameters $a_1$.
% > * **Branch 3 (Representation):** Show $z$ passing into an auxiliary MLP Regression head to predict the $17 \times 2$ dimensional Semantic Keypoints (include a mask icon indicating gradients are only applied during simulation data batches).
...
$$\mathcal{L}_{para}^{\omega}(\phi) = \mathbb{E}_{\substack{k \sim \mathcal{U}[0,1] \\ a_0 \sim \mathcal{N}(\bm{0}, \bm{I}) \\ a_1 \sim \mathcal{D}^\omega}} \left[ \| v_{\phi}^{\omega}(a_k^\omega, k \mid z) - (a_1 - a_0) \|^2 \right]$$ \textbf{TODO: fix the action parameter}
...
All network components—including the shared visual encoder, the representation regressor, the primitive classification network, and the suite of flow-matching action generative models—are optimised jointly in an end-to-end manner. The overall training objective is formulated as a weighted sum of the primitive classification loss $\mathcal{L}_{prim}$, the representation learning loss $\mathcal{L}_{rep}$, and the sum of the optimal transport flow-matching losses across all active primitive networks $\sum_{\omega \in \Omega} \mathcal{L}_{para}^{\omega}$:
$$\mathcal{L}_{total} = \beta_{prim} \mathcal{L}_{prim} + \beta_{para} \sum_{\omega \in \Omega} \mathcal{L}_{para}^{\omega}$$
where the scaling coefficients are empirically set to $\beta_{prim} = 0.001$ and $\beta_{para} = 1.0$ to balance the auxiliary objectives. We employ the AdamW optimiser and apply weighted cross-entropy to handle the highly imbalanced distribution of primitives within the demonstration dataset. 
...
\textbf{TODO: explain brief how MAGPIE works briefly and refer the reader to read the main body seciton first.} Our implementation relies on a highly decoupled architecture to manage the multi-primitive nature of garment shaping. 

\subsection{Shared Vision Encoder}
To process visual inputs, we utilise a ResNet-18 backbone. \textbf{TODO: ciation} Because standard Batch Normalisation performs poorly with the small batch sizes typical of continuous robotic control algorithms, we replace all 2D Batch Normalisation layers with Group Normalisation (\texttt{num\_groups}=16). The final fully connected layer is removed, and an Adaptive Average Pooling layer compresses the spatial features into a 512-dimensional vector. 

To refine these features for downstream tasks, this vector is subsequently passed through an MLP projector consisting of two hidden layers with dimensions $[256, 256]$ and ReLU activations, outputting a final 256-dimensional visual embedding. During training, a feature noise factor of $0.01$ is applied to the observation features to improve robustness. For state-inclusive experiments, physical vector states are concatenated with this projected visual embedding. 

\subsection{Primitive Classification Head}
The high-level primitive selection is handled by an MLP classifier. The network takes the 256-dimensional projected visual embedding and passes it through a single hidden layer with a dimension of 128. We apply a ReLU activation function, followed by Layer Normalisation and a Dropout layer with a probability of $p = 0.5$. The final output maps to the $K=4$ available action primitives. To ensure balanced decision-making during training, the classes are equally weighted ($\lambda_\omega = 1.0$ for all classes).
...
Batch Size & 256 \\
Total Update Steps & 100,000 \\
Trial Validation Interval & 100 \\
...
Observation Embedding Dim & 256 (Projected) \\
Input Modalities & \texttt{rgb} + \texttt{goal\_rgb} \\
Input Image Resolution & $96 \times 96$ \\
...
Primitive Loss Weight ($\beta_{prim}$) & 0.001 \\
\midrule
\multicolumn{2}{c}{\textit{Representation Learning}} \\
\midrule
Representation Mode & Predict State (\texttt{predict-state}) \\
Representation Loss Weight ($\beta_{rep}$) & 0.5 \\
State Dimension (Keypoints) & 34 ($17 \times 2$) \\
Goal State Prediction & Disabled \\
\bottomrule
\end{tabular}
\end{table}
...
For each of the $K$ primitives, we instantiate an independent 1D Conditional U-Net backbone to regress the optimal transport vector field. This separate-network integration ensures that distinct robotic behaviours do not suffer from parameter interference.
\begin{itemize}
    \item \textbf{Architecture Backbone:} The U-Net employs a sequence of `ConditionalResidualBlock1D` modules. We set \texttt{disable\_updown=True}, effectively turning the architecture into a deep sequence of 1D convolutional residual blocks without spatial downsampling, which preserves the strict dimensionality of the continuous action spaces ($4, 8, 4$, or $0$). Each convolutional block consists of a 1D Convolution, Group Normalisation, and a Mish activation function.
    \item \textbf{Timestep Embedding:} The continuous flow timestep $k$ is encoded using Sinusoidal Positional Embeddings, which are then passed through a two-layer MLP sequence (Linear $\rightarrow$ Mish $\rightarrow$ Linear) to generate the temporal feature.
    \item \textbf{Conditioning:} The global visual context $z$ is concatenated with the temporal feature. This combined embedding is injected into the residual blocks using Feature-wise Linear Modulation (FiLM). Specifically, a conditioning encoder (Mish $\rightarrow$ Linear) predicts a per-channel scale and bias to dynamically modulate the network's activations after the first convolutional block.
    \item \textbf{Integration:} During training, the flow matches the constant velocity field $a_1 - a_0$. During inference, we utilise an ODE solver with 10 integration steps to reliably transport the initial Gaussian noise into the predicted spatial parameters.
\end{itemize}

\subsection{Representation Regressor Head}
The auxiliary semantic keypoint predictor is an MLP regression head consisting of two hidden layers with dimensions $[256, 256]$, featuring ReLU activations, Layer Normalisation, and a dropout probability of $0.5$. It takes the 256-dimensional projected visual embedding and maps it directly to a 34-dimensional vector, representing the maximum 17 2D semantic keypoints defined in our framework. Goal state prediction via this network is explicitly disabled in this configuration.

\subsection{Optimization Hyperparameters}
We optimise all networks end-to-end using the AdamW optimiser with a base learning rate of $1 \times 10^{-4}$ and a weight decay of $1 \times 10^{-2}$. We employ a cosine annealing learning rate scheduler with a warmup period of 1,500 steps. To stabilise training, we apply gradient clipping and maintain an Exponential Moving Average (EMA) of the model weights with a high decay rate of 0.9999.
