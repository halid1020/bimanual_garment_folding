...
\includegraphics[width=1.0\linewidth]{plots/combined_garments_4x4_with_goals.png}\hfill
    ...
    \caption{Overview of LaGarNet, trained in simulation \cite{canberk2023cloth} and transferred zero-shot to a UR5e with the \text{DRAPER} framework \cite{kadi2025draper}. (a) Initial and goal states of the simulated \text{Cloth3D} garments \cite{bertiche2020cloth3d}. (b) Real garments used for physical evaluation, whose shapes and materials are out of the training distribution. (c) The UR5e with an OnRobot RG2 gripper. (d) LaGarNet flattening a real dress: bright blue arrows are the mean of the action Gaussian in the early CEM iterations and purple arrows the resulting final ones (top); the bottom row reports normalised coverage (NC), maximum IoU, the ground-truth reward and the prior reward predicted at the last planning step, all multiplied by 100.}
    \label{fig:LaGarNet}
\end{strip}

\begin{abstract} Manipulating cloth is a grand challenge in robotics: its many degrees of freedom yield an enormous state-action space that makes learning-based control extremely difficult to train. We present a goal-conditioned recurrent state-space model (GC-RSSM) that learns the latent dynamics of pick-and-place garment manipulation. Unlike the velocity- and position-controlled settings in which latent world models are usually studied, high-level pick-and-place is quasi-static: consecutive observations are not temporally continuous, and each transition is a single large deformation. We show that a recurrent latent world model nonetheless learns useful dynamics in this regime, and that our method LaGarNet matches the state-of-the-art performance of mesh-based methods on the difficult task of garment flattening whilst using five times fewer parameters and inferring over an order of magnitude faster---the first successful application of state-space models to complex garments. LaGarNet trains on a coverage-alignment reward and on data collected by a general procedure combining a random policy with a diffusion policy learned from a few human demonstrations, and a single policy flattens four garment types in both simulation and the real world.
\end{abstract}
...
Robotic manipulation of cloth-like deformable objects is a significant technical challenge with profound societal and economic implications. Automating it is essential for modernising garment manufacturing \cite{nayak2018introduction}, for assisting elderly and mobility-impaired populations \cite{WHOaged2024, disablity2011world}, for domestic laundry, and for sterile handling in clinical settings \cite{cornish2021clinical}. It remains difficult due to the complex, underactuated dynamics of cloth, and garment flattening is a foundational milestone preceding tasks such as folding, ironing and pressing \cite{doumanoglou2016folding}.

Model-based methods learn and utilise a world model (i.e., the dynamics) of the environment \cite{ha2018world, hafner2019learning}, but cloth's large degrees of freedom make such controllers extremely difficult to train \cite{kadi2023data}, and world models have remained elusive here due to the difficulty of capturing accurate deformation and self-occlusion \cite{ma2021learning,yan2021learning,lin2020softgym}.

We present LaGarNet, a general single-policy method capable of flattening complex garment types (Figure \ref{fig:LaGarNet}). It mitigates the strong inductive biases of previous data-driven cloth manipulation methods \cite{kadi2025draper, weng2022fabricflownet, hoque2022visuospatial}, which rely on domain-specific architectures, reward functions and datasets from a particular blend of expert and random oracle policies, and generalise poorly to complex garments since each type needs its own engineered oracle. LaGarNet instead couples a goal-conditioned recurrent state-space model (GC-RSSM) with a general data-collection procedure supported by an expert proximal Diffusion Policy \cite{chi2023diffusionpolicy} and a coverage-alignment reward.

We adopt a top-down camera view and a single-gripper pick-and-place (PnP) primitive. Our human-in-the-loop experiments show operators flatten diverse garments exceptionally well with single-arm commands alone, indicating the primary bottleneck is physical reasoning rather than hardware. Restricting ourselves to one gripper isolates this cognitive challenge and focuses the work on representation learning, whilst bypassing the complexities of bimanual coordination and multi-primitive control \cite{canberk2023cloth}, at the cost of more iterations.
...
2. We develop a general data-collection strategy for offline training of \text{PnP} garment-flattening controllers, mixing an expert \text{Diffusion Policy} learned from few demonstrations with a mask-biased random policy.

3. We design a coverage-alignment reward function for general garment flattening, computable in both simulation and the real world.

4. We develop a sampling and transfer heuristic adapting the policy from its fixed square observation window to a robot arm's ring-shaped workspace.
...
The mesh-free \text{Deep Planning Network (PlaNet)} \cite{hafner2019learning} has been widely studied in control tasks \cite{yan2021learning, ma2021learning, lin2020softgym}, but shows limited efficacy on deformable objects, attributed to blurred visual reconstructions that lose the edge and corner details critical for shaping \cite{yan2021learning, lin2022learning}. \text{PlaNet-ClothPick} \cite{kadi2024planet}, a cloth-flattening algorithm based on a recurrent state-space model (RSSM), mitigates this by restricting pick actions to the cloth mask, designing a flattening-specific reward and using an engineered offline dataset; it achieves perfect flattening in simulation and over 70\% NC on real towels \cite{kadi2024planet, kadi2025draper}, but fails to generalise to complex garments due to these strong inductive biases. Our single-policy \text{LaGarNet} reduces them through a general data-collection procedure and a coverage-alignment reward.

Lastly, model-based methods derive reward signals from the difference between current and goal observations \cite{hoque2022visuospatial, yan2021learning}. Arnold et al.~\cite{arnold2023cloth} extend this to mesh-level goal-conditioned policies for complex tasks, but remain limited to fabric manipulation. Model-free approaches train goal-conditioned representations \cite{mo2022foldsformer}, flow networks \cite{weng2022fabricflownet}, or use a \text{Hindsight Replay Buffer} \cite{andrychowicz2017hindsight, lee2021learning}, yet are also restricted to simple fabrics. We instead propose a goal-conditioned RSSM (GC-RSSM) that predicts step-wise flattening rewards and flattens four garment types: longsleeved T-shirts, trousers, skirts and dresses.
...
The high-dimensional state-action space of deformable objects makes policy representations, dynamics models and value functions hard to train, and model-based cloth-shaping methods further suffer from model exploitation (choosing actions the model incorrectly estimates as optimal) and compounding error in imagined roll-outs. The effectiveness of a learned world model depends on how diverse and comprehensive the training trajectories are \cite{duan2024learning}, and training typically requires large amounts of data collected with carefully designed blends of policies \cite{hoque2022visuospatial, yan2021learning, kadi2024planet}. Prior work has used demonstration data \cite{matas2018sim}, corner-biased sampling that steers pick actions towards fabric corners and edges \cite{hoque2022visuospatial,weng2022fabricflownet}, and other engineered strategies \cite{arnold2023cloth,lin2022learning}---all of which impose strong inductive biases on the learning algorithm. We instead propose a general data collection procedure combining a Diffusion Policy \cite{chi2023diffusionpolicy} trained from a few human demonstrations with a random oracle policy, which empirically yields effective training data for MBRL algorithms.


\subsection{Goal-Conditioned Methods using RSSM}
Previous research has integrated goal-conditioned learning into RSSMs. Pertsch et al.~\cite{pertsch2020long} propose a goal-condition predictor \text{(GCP)} conditioned on the initial and goal states of a subtask of a long-horizon task, improving planning with a hierarchical divide-and-conquer scheme in which the action is inferred inversely from the current state and an optimised sub-goal. Duan et al.~\cite{duan2024learning} present the goal-directed exploration algorithm \text{MUN} and devise \text{GC-Dreamer} as a baseline, but condition its goal on the actor and critic networks instead of the world model. We differ by conditioning the goal directly on the latent state inference of the RSSM, which learns a better representation.
 ...
 \path (yt2) -- node {\dots} (yT); \path (rt2) -- node {\dots} (rT);
    ...
    \begin{subfigure}[b]{0.32\linewidth}
        ...
        \scalebox{0.90}{\begin{tikzpicture}[
            ...
            \node[box, fill=green!20] (demo) {Human demos $\mathcal{D}_{\text{demo}}$};
            \node[dec, below=of demo, fill=gray!15] (dec) {$u\sim\mathcal{U}(0,1)$};
            \node[box, below left=6mm and -6mm of dec, fill=blue!15]  (diff) {Diffusion policy\\$\pi_{\text{diff}}$};
            \node[box, below right=6mm and -6mm of dec, fill=orange!20] (rdm) {Mask-biased random\\$\pi_{\text{rdm}}$};
            \node[box, below=14mm of dec, fill=white] (unroll) {Unroll trajectory $\tau$};
            \node[box, below=of unroll, fill=purple!20] (data) {$\mathcal{D}\leftarrow\mathcal{D}\cup\{\tau\}$};

            \draw[->, thick] (demo) -- node[lbl, right] {train} (dec);
            ...
            \draw[->, thick] (data.east) -- ++(6mm,0) |- node[lbl, right, pos=0.25] {$\times M$} (dec.east);
        \end{tikzpicture}}
...
\subfloat[Longsleeved T-shirt flattening against SoTA baselines.]{%
    \includegraphics[width=0.64\linewidth]{plots/lagarnet_flattening_heatmap.png}%
    \label{fig:longsleeve-flattening}}\hfill
\subfloat[All-garment generalisation by action step 20.]{%
    \includegraphics[width=0.33\linewidth]{plots/multi_arena_radar_2x2.png}%
    \label{fig:sim-lagarnet-results}}
\caption{Simulation results for single-arm pick-and-place (\text{PnP}) controllers. \textbf{(a)} Per-horizon longsleeve flattening: for each metric the columns report the cumulative \emph{Max} (best value reached within the horizon) and the \emph{Last} (final-step) value at 5, 10, 20 and 30 action steps; cell colour encodes the score in $[0,1]$. Only the single-arm PnP primitive of \text{ClothFunnels}~\cite{canberk2023cloth} is enabled; \text{Diffusion Policy}~\cite{kadi2025draper, chi2023diffusionpolicy} is trained on 200 human demonstrations, and \text{MEDOR}~\cite{huang2022mesh} is evaluated in its own simulation setup. All-Garment \text{LaGarNet} is comparable to MEDOR at a small fraction of its inference cost and competitive with human experts at intermediate horizons, although humans remain ahead at 30 steps; the small Max$\to$Last gap indicates that final configurations remain stable. \textbf{(b)} Performance on all four garment types by action step 20, where \text{LaGarNet} consistently outperforms all-garment \text{PlaNet-ClothPick} and task-specific \text{Diffusion Policy}.}
...
Using the setup of Section \ref{sec:simulation}, we investigate whether LaGarNet's advantage over Diffusion Policy stems from explicit forward planning, from extended temporal context, or simply from data scale: we examine how scaling human demonstrations affects a task-specific Diffusion Policy (Figures \ref{fig:diffusion-human-val} and \ref{fig:diffusion-human-eval}), design ablated baselines (Section \ref{sec:diff-baselines}), and analyse their primary metrics (Section \ref{sec:diff-result}). 


\subsubsection{Baselines} \label{sec:diff-baselines}

To isolate the specific architectural components that contribute to LaGarNet's superiority over Diffusion Policies, we design the following baseline policies: (a) \textbf{GC-DP}: a Goal-Conditioned Diffusion Policy (Appendix \ref{sec:diffusion}) taking the concatenated current and goal RGB images as input, trained on 5,000 episodes of diffusion-induced, success-only data spanning all garment types. (b) \textbf{GC-DP-Comp}: GC-DP with hyperparameters aligned to LaGarNet for a fair comparison: the ResNet-18 encoder is replaced by PlaNet's CNN encoder, the current and goal observation embeddings are concatenated before the downstream networks, and the observation sequence length is raised from 1 to 4. (c) \textbf{LaGarNet-Comp}: LaGarNet with hyperparameters constrained for direct comparison against the diffusion baselines, most significantly a training sequence length reduced from 50 to 4. 

We also have (d) \textbf{LaGarNet-RSSM}: The standard LaGarNet architecture utilising its full RSSM dynamics model. (e) \textbf{GC-RSSM-DP}: GC-DP-Comp using LaGarNet's pre-trained latent dynamics model, with the noise predictor's conditioning dimension set to 360. At each step the historical memory is wiped by zeroing the previous latent state, and the current posterior latent state is obtained by feeding a ``no-op'' action into the dynamics model; both the vision encoder and the dynamics model of the pre-trained GC-RSSM are frozen during training. (f) \textbf{LaGarNet-CNN-DP}: GC-DP-Comp using LaGarNet's frozen pre-trained vision encoder, giving a 2048-dimensional observation embedding formed by concatenating LaGarNet's current and goal embeddings. We train all policies using the appropriate hyperparameters detailed in Table \ref{tab:all-hyperparameters}.c, applying consistent data augmentation schemes across all methods (Appendix \ref{sec:data-aug}).

\subsubsection{Results} \label{sec:diff-result}

As detailed in Table \ref{tab:comp-diff-lagarnet}, LaGarNet substantially outperforms the diffusion baselines, reaching a success rate of 72.5\% against GC-DP's 40.0\%---notable given that LaGarNet trains on 5,000 episodes of mixed-quality transitional data whereas GC-DP uses an equivalent volume of strictly success-only trajectories. LaGarNet leads on every metric, peaking at 96.1 $\pm$ 1.1\% NC and 91.3 $\pm$ 2.3\% NI, suggesting that its model-based architecture is better suited to garment flattening. 
The ablated variants underscore the importance of LaGarNet's extended temporal context: constraining it to a sequence length of 4 (LaGarNet-Comp) collapses the success rate to 5.0\%, indicating that a world model trained over a long context horizon is essential for the long-horizon dynamics of deformable objects. Reverting to the original RSSM (LaGarNet-RSSM) reduces the success rate by around 30\%, confirming the importance of goal conditioning.

Transplanting LaGarNet's pre-trained visual or latent representations into the diffusion framework (LaGarNet-CNN-DP and GC-RSSM-DP) yields success rates of only 8.3\% and 6.7\%. This exposes a fundamental limitation of the diffusion baselines: their efficacy rests almost entirely on an end-to-end, reactive mapping from observations to action noise rather than on a structured understanding of environment dynamics, so without the capacity to reason explicitly over future states they cannot exploit these rich pre-trained representations. LaGarNet instead uses its decoupled world model to anticipate and plan for complex non-rigid deformations, indicating that its model-based paradigm, grounded in explicit forward planning, is far better suited to long-horizon garment manipulation than the purely reactive, data-hungry diffusion models.
...
\begin{figure*}[t!]
    ...
    \begin{minipage}[t]{0.52\textwidth}
        ...
        \subfloat[\centering Latent Dynamic and Goal Ablation \label{fig:goal-ablation}]{
            \includegraphics[width=0.40\linewidth]{plots/ablation_latent_bar.png}
        }
        \subfloat[\centering Reward Ablation \label{fig:reward-ablation}]{
            \includegraphics[width=0.40\linewidth]{plots/ablation_reward_bar.png}
        }        
        ...
        \subfloat[\centering Data Size Ablation. \label{fig:datasize-ablation}]{
            \includegraphics[width=0.40\linewidth]{plots/ablation_data_size_line.png}
        } 
        \subfloat[\centering Data Composition Ablation \label{fig:dataratio-ablation}]{
            \includegraphics[width=0.40\linewidth]{plots/ablation_data_ratio_bar.png}
        } 
...
\begin{table}[t!]
\caption{RGB image reconstruction quality of different world models, measured by Mean Squared Error (MSE) and Structural Similarity Index (SSIM) \cite{wang2004image}; SSIM is informative here because it captures the local structure and contrast that blurring degrades. All values are multiplied by 100 and averaged over 10 trials for each of the 4 garments. The models share the same internal hyperparameters and differ only in structure and in their inputs and outputs. Our GC-RSSM reduces blurriness in the multi-step prior reconstructions.}
...
\end{table}
We examine the effects of LaGarNet's components---the latent dynamics structure, the reward function, the data size, the dataset mixture ratio and the planning horizon---in the simulation setup (Section \ref{sec:simulation}). Except for the latent dynamics ablations, which use RGB-to-RGB variants to inspect prediction capability (Figure \ref{fig:recon-quality}), all experiments use the default RGB-to-Mask variants.

\subsubsection{Ablation Baselines}


Alongside the \textbf{GC-RSSM} and \textbf{RSSM} models of Section \ref{sec:gc-rssm}, the latent dynamics and goal ablation evaluates \textbf{RSSM-GC-I}, which concatenates the goal RGB image with the current one as input but decodes only the current observation, and \textbf{RSSM-GC-IO}, which also predicts the goal image. This isolates whether LaGarNet benefits from goal-conditioning itself or from the structural differences between GC-RSSM and RSSM. The reward ablation replaces LaGarNet's reward with the alternatives of Section \ref{sec:reward-literature}: Learning2Unfold (\textbf{L2U}) \cite{gu2023learning}, ClothFunnels (\textbf{CF}) \cite{canberk2023cloth}, PlaNet-ClothPick (\textbf{PC}) \cite{kadi2024planet} and an approximation of SpeedFolding (\textbf{SFA}) \cite{avigal2022speedfolding}. The dataset ablations vary both the volume and the composition of the training data, including diffusion-only and random-only datasets and an online LaGarNet that follows PlaNet's exploration strategy \cite{hafner2019learning} by adding Gaussian noise to the MPC output. \textbf{TODO: talk about the ablation baselines.}
 
\subsubsection{Results}

LaGarNet's advantage stems from the combination of its GC-RSSM latent dynamics, its coverage-alignment reward and its data collection process. Figure \ref{fig:goal-ablation} shows the advantage arises not only from goal-conditioning but from the structure of GC-RSSM: RSSM-GC-I performs similarly, whilst RSSM-GC-IO, which reconstructs both current and goal observations, performs much worse, likely because of the extra learning burden. Figure \ref{fig:recon-quality} and Table \ref{tab:recon-quality} further show that GC-RSSM yields a superior prior latent state representation, which is crucial for planning and hence control performance.
...
\begin{figure}[t!]
\centering
% TODO: replace the placeholder box below with the real figure once the sweep finishes.
% \includegraphics[width=\linewidth]{plots/ablation_mpc_horizon.png}
\fbox{\parbox[c][0.16\linewidth][c]{0.94\linewidth}{\centering\color{gray}\footnotesize\textsc{Placeholder}: \texttt{plots/ablation\_mpc\_horizon.png}}}
\caption{Ablation on the MPC planning horizon $H$. \textbf{TODO: results pending.}}
\label{fig:horizon-ablation}
\end{figure}

The planning horizon $H$ trades control quality against computation: a longer horizon lets the CEM planner account for multi-step consequences of a pick-and-place, but the number of transition-model rollouts---and hence the per-step planning time---grows proportionally. Figure \ref{fig:horizon-ablation} will report normalised coverage, normalised improvement and per-step planning time for $H \in \{1,2,3,5\}$ under the same evaluation protocol as above, together with the unconstrained-CEM variant that plans without the cloth and workspace mask rejection. \textbf{TODO: this sweep has not yet been run; the figure and the accompanying discussion are placeholders.}
...
\begin{figure*}[t!]
    ...
    \begin{subfigure}[t]{0.47\textwidth}
        ...
        \centering
        \includegraphics[width=0.90\linewidth]{plots/lagarnet-failure.jpg}
...
Our framework assumes that intrinsic physical properties (e.g., stiffness, elasticity) are secondary to geometric representations for high-level policy learning, so LaGarNet struggles with garments of high internal friction and low stiffness (Table \ref{tab:unseen-dress}); the single-arm parallel-jaw gripper compounds this by limiting the ability to unfold stubborn folds and wrinkles. Distinguishing a garment's front from its back was also not a task criterion, so LaGarNet cannot perform this semantic orientation---a spatial task known to be challenging even for young children \cite{paoletti2012pink}. Finally, time consumption is a critical limitation for deployment in fast-paced garment factories. Figure \ref{fig:real_world_analysis}(c) shows that perception (8.19\,s) and action inference (7.05\,s) form a substantial computational bottleneck; with robot execution (9.20\,s) a single manipulation step takes about 24.5\,s, i.e. over 8 minutes for a 20-step episode. Scaling to industrial or domestic use therefore requires optimising the latent dynamics model's inference speed and the perception pipeline.

\section{Discussion} \label{sec:discussion}

Our central finding is that a recurrent latent world model remains effective in a \emph{quasi-static} setting. Applying RSSM to pick-and-place cloth shaping poses two critical challenges: capturing the complex deformation of cloth, and the quasi-static nature of high-level primitives---inferred states are not as continuous as in velocity- or position-based control, so each transition is a single large deformation rather than a smooth increment. Latent world models are usually studied in the latter regime, and it was not obvious they would transfer. We find that \text{RSSM} \cite{hafner2019learning} with its goal-conditioned extension does learn the latent dynamics of cloth deformation under high-level primitives, and that planning in this compact latent space is both far cheaper than and competitive in accuracy with planning over an explicitly reconstructed mesh.

Offline RL generally suffers from distribution shift \cite{prudencio2023survey}, where test trajectories misalign with the training data, and most of the literature relies on datasets collected by a single policy \cite{agarwal2020optimistic}. We find that the behaviour cloning (BC) method \text{Diffusion Policy} \cite{chi2023diffusionpolicy} learns to flatten garments from only 50--200 human demonstrations and, producing diverse actions around the expert distribution, can then collect near-success data for offline RL. Combining a random policy with such a learned diffusion policy is therefore a potentially generalisable data-collection recipe.

LaGarNet is computationally cheaper than mesh-based planning, reaching comparable performance with fewer parameters and faster inference, and is more data-efficient and stable than model-free and online RL. Against BC, its policy is inherently explainable---it predicts the outcome of a candidate action---and is not limited by demonstration quality, whereas BC relies on extensive expert data that is laborious to collect without an oracle: 200 episodes of 20-step trajectories cost roughly nine hours of human labour per garment type.

BC methods also transfer poorly. In our physical setup the workspace constraints fluctuate with where the heuristic crops the raw RGB image, and a Diffusion Policy trained without those exact constraints cannot easily be transferred, as enforcing workspace masks during action denoising is highly complex; training for all real-world variability would demand an impractical amount of data. LaGarNet handles this natively by constraining sampled actions to the workspace during planning, so a policy trained with loose constraints in simulation transfers to real setups with varying constraints without retraining a simulated twin.

This opens an exciting direction for applying state-space models to broader deformable object manipulation---dough, clay or cables---whose complex dynamics and high degrees of freedom can be encoded into a latent space that accelerates planning and reasoning. Whilst we focus on reward-based planning for flattening, the model's future-feature prediction could be extended to goal-directed folding.
