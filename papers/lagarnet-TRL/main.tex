...
\includegraphics[width=1.0\linewidth]{plots/combined_garments_4x4_with_goals.png}\hfill
    ...
    \caption{Overview of LaGarNet. \rev{\rrev{\textbf{(a)}} Initial and goal states of simulated \text{Cloth3D} garments \cite{bertiche2020cloth3d} in our benchmark. \rrev{\textbf{(b)}} Real garments used for physical evaluation, \irev{where the shape and material of the cream longsleeved dress (marked \emph{Out of Distribution}) are unseen during training}. \rrev{\textbf{(c)} Our single-arm real-world setup with a parallel gripper; input images are taken by a top-down camera.} \rrev{\textbf{(d)}} LaGarNet flattening a real dress\rrev{: blue arrows are early Model Predictive Control action means, purple the final ones (top); the bottom row reports Normalised Coverage (NC), Normalised Improvement (NI), Maximumn Interserction over Union (Max IoU), and the true and predicted prior rewards ($\times$100)}. Trained in simulation, our single-policy LaGarNet transfers zero-shot to the real world for flattening 4 different types of garments.}}
    \label{fig:LaGarNet}
\end{strip}
...
\begin{figure*}[t!]
 ...
 \path (yt2) -- node {\dots} (yT); \path (rt2) -- node {\dots} (rT);
    ...
    \caption{\rev{LaGarNet: \rrev{\textbf{(a)}} the probabilistic graphical model and \rrev{\textbf{(b)}} the network architecture; matching colours, line styles, blocks, and nodes denote identical processes. $\bm{g}$ is the goal observation, $\bm{x}_t$ and $\bm{z}_t$ the observation and latent state before action $\bm{a}_t$, and $\bm{h}_t$ the hidden state of the recurrent backbone. \rrev{In (a), purple observation and grey action nodes are model inputs; yellow action nodes are sampled during planning. In (b), the \textit{Prior} (red, solid) predicts $\boldsymbol{\tilde{z}}$ from $\boldsymbol{h}$ and $\boldsymbol{g}$, the \textit{Posterior} (orange, dashed) additionally integrates $\boldsymbol{x}$, and a joint decoder (blue) emits $\boldsymbol{\hat{y}}$ and $\boldsymbol{\hat{r}}$ (Section \ref{sec:gc-rssm}).} \rrev{\textbf{(c)}} Sensitivity of the coverage-alignment reward $\mathcal{R}_{CA}$ to the interpolation coefficient $\alpha_r$. \rrev{\textbf{(d)}} Data collection: a Diffusion Policy expert is mixed at ratio \rrev{$\xi$} with the mask-biased random policy to unroll the $M$ trajectories of the offline dataset $\mathcal{D}$.}}
    \label{fig:lagarnet}
\end{figure*}
...
\caption{\rev{Simulation results for single-arm pick-and-place (\text{PnP}) controllers. \textbf{(a)} Per-horizon longsleeve flattening: \rrev{Rows are methods; each horizon $N \in \{5, 10, 20, 30\}$ spans a \emph{Max} column (best within $N$ steps) and a \emph{Last} column (value at step $N$).} LaGarNet macthes the primary performance of mesh-based planner MEDOR; whilst Normalised Coverage (NC) and Maximum Intersection over Union (Max IoU) of LaGarNet barely change between Max and Last, Success Rate (SR) drops sharply (Section \ref{sec:discussion}). \textbf{(b)} Performance on all four garment types by action step 20, where \text{LaGarNet} consistently outperforms all-garment \text{PlaNet-ClothPick} and task-specific \text{Diffusion Policy}.}}
...
\begin{figure*}[t!]
    ...
    \caption{Comprehensive ablation study for LaGarNet in simulation environments. \textbf{(a, b, c, and d)} Ablation study using the all-garment LaGarNet models\rrev{.} \textbf{(e)} Observation reconstruction quality of models from Figure (a); each grid is a 64$\times$64 RGB image; \rrev{green arrows in the first row are pick-and-place actions. GC-RSSM tracks garment sleeves in prior steps better than the other RSSM variants.} \rev{\textbf{(f)} Flattening performance against the MPC planning horizon $H$, reported both cumulatively (\emph{Max}) and at the final step; hollow markers report the unconstrained-CEM variant at $H=1$. \textbf{(g)} The reconstructions of (e) scored by Mean Squared Error (MSE) and Structural Similarity Index (SSIM) \cite{wang2004image}, both $\times$100 and averaged over 10 trials per garment\rrev{; provided that SSIM captures the local structure that blurring degrades, GC-RSSM blurs the multi-step prior reconstructions least.}}}
...
\rev{Our LaGarNet method is an integrated world-model system. \rrev{To our knowledge,} it is the first latent world model that flattens four garment topologies with a single policy in a quasi-static setup. We study latent dynamic representations because they promise generalisability beyond the laboratory \cite{ha2018world}. Applying an RSSM here raises two challenges: cloth deformation is complex, and high-level primitives make inferred states far less continuous than in velocity or position control. We find that RSSM \cite{hafner2019learning}, with our goal-conditioned extension, still learns the latent dynamics of cloth deformation in quasi-static setups.}
...
% Main body + references must end here; the appendix starts on its own page.
\clearpage
