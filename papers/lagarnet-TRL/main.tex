...
\author{
Halid Abdulrahim Kadi$^{1,2}$,~\IEEEmembership{Member,~IEEE},~\orcidlink{0000-0001-9290-467X},
John Oyekan$^{1,3}$,~\IEEEmembership{Member,~IEEE},~\orcidlink{0000-0001-6578-9928}
, and
...
\thanks{$^{3}$~Loughborough University.}
...
\rev{A supplementary document accompanied with the full architecture, hyperparameters, planning procedure and real-robot setup, together with video material showing real-world rollouts of LaGarNet flattening the four garment types.}
...
\rev{SpeedFolding \cite{avigal2022speedfolding} uses a coverage-smoothness reward $\mathcal{R}_{CS} = \max(\tanh(\alpha \cdot \mathcal{R}_{dC} + \beta \cdot \tilde{\mathcal{R}}_{dS}), 0)$; it mixes the delta coverage $\mathcal{R}_{dC} = NC_t - NC_{t-1}$ with a delta smoothness estimate $\tilde{\mathcal{R}}_{dS}$ between consecutive frames, inferred by a pre-trained network. PlaNet-ClothPick adapts VSF's fabric-flattening reward \cite{hoque2022visuospatial}. We propose a novel coverage-alignment reward $\mathcal{R}_{CA}$ that linearly combines coverage and the IoU difference between the last two frames, computing both from top-down segmentation masks alone, so one reward serves simulation and reality. It simplifies SpeedFolding's $\mathcal{R}_{CS}$ and, unlike ClothFunnels' reward, requires no particle-wise distances.}
...
\rev{LaGarNet trains a goal-conditioned latent dynamics model by learning a feature representation $\bm{y}$, extending the RSSM with goal-aware inference and generation (Figure~\ref{fig:gc-rssm-pgm}): the goal observation $\bm{g}$ enters the estimation of the stochastic latent state $\bm{z}$, defining the GC-RSSM as:}
\begin{align}
...
\rev{As shown in Figure \ref{fig:network}, LaGarNet comprises four components: a convolutional image encoder, a transposed-convolutional decoder, a goal-conditioned transition model built from MLPs and a Gated Recurrent Unit \cite{cho2014properties}, and a 5-layer MLP reward predictor, with ReLU activations throughout. The training objective maximises the marginal log-likelihood of the estimated features and rewards given the action sequence and the target goal observation:}
\begin{equation}
    \argmax_{\theta} \ln p_{\theta}(\bm{y}_{1:T}, r_{1:T} | \bm{a}_{1:T}, \bm{g}). \label{eqn:obj}
\end{equation}
...
\end{equation}
\rev{where $\text{sg}(\cdot)$ is the stop-gradient operator, the balancing coefficient is $\beta_{KL} = 0.8$, and the term is scaled by $\gamma_{KL} = 1.0$ in the total loss. To enforce consistent multi-step future predictions, we add overshooting losses---computed from more than one-step priors---on the KL divergence and reward predictions of future prior latent states across $T_f$ steps, weighted by $\gamma_{KLo} = \gamma_{ro} = 0.1$.}

\rev{During training, we stop gradients only for the overshooting loss, never for the goals; GC-RSSM prevents the recurrent structure from ``forgetting'' the goal. The total loss is then:}
\begin{equation}
    \mathcal{L}_{\text{total}} = \gamma_y \mathcal{L}_y + \gamma_r \mathcal{L}_r + \gamma_{KL} \mathcal{L}_{KL} + \gamma_{KLo} \mathcal{L}_{KLo} + \gamma_{ro} \mathcal{L}_{ro}, \label{eqn:total-loss}
\end{equation}
\rev{where $\mathcal{L}_y$ and $\mathcal{L}_r$ are the MSE losses for feature and reward estimation, and $\mathcal{L}_{KLo}$ and $\mathcal{L}_{ro}$ are the corresponding KL and reward overshooting losses over $T_f$ steps.}
...
\end{align}
where $\tilde{\bm{z}}_{k+1}(\bm{a}_k) \sim p(\tilde{\bm{z}}_{k+1} \;|\; \bm{h}_{k+1}=f(\bm{h}_k, \hat{\bm{z}}_k, \bm{a}_k), \bm{g})$. In our garment flattening setup, we set the prior future prediction horizon to $T_f=1$: Section \ref{sec:abl} shows that longer horizons degrade performance, because a single primitive produces one large deformation and prediction error compounds across a multi-step rollout. \rev{CEM solves this over the normalised action space $[-1,1]^4$, whose dimensions encode the pick and place positions in normalised pixel coordinates. We initialise a diagonal Gaussian with $\bm{\mu}=\bm{0}$ and $\bm{\sigma}=\bm{1}$, then run $K=100$ iterations. Each iteration draws $N=5000$ candidates and rejects, without resampling, any that violate the masks. A pick pixel must lie inside both the cloth mask and the robot's workspace mask, and a place pixel inside the workspace mask. The planner then scores the survivors with $r_\theta$ and refits $\bm{\mu}$ and $\bm{\sigma}$ on the best $10\%$ ($N_e=500$ elites), and finally executes the clipped mean of the last distribution. We sample and score every candidate in normalised image coordinates, and convert only the executed action into a metric robot pose.}

\subsection{Coverage-Alignment Reward}  \label{sec:ca-reward}

\rev{Our coverage-alignment reward $\mathcal{R}_{CA}$ combines the step-wise normalised coverage difference $\mathcal{R}_{dC}$ with the step-wise difference in maximum IoU between the current and goal cloth masks, $\mathcal{R}_{dU}$:} 

\begin{equation}
    \mathcal{R}_{CA} = \alpha_r \cdot \mathcal{R}_{dC} + (1-\alpha_r) \cdot \mathcal{R}_{dU},
\end{equation}
\rev{where $\mathcal{R}_{dC} = \text{NC}_t - \text{NC}_{t-1}$ and $\mathcal{R}_{dU} = \text{MaxIoU}_t - \text{MaxIoU}_{t-1}$; both $\mathcal{R}_{dC}$ and $\mathcal{R}_{dU} \in [-1, 1]$, so that $\mathcal{R}_{CA} \in [-1, 1]$. In our experiments, we set $\alpha_r$ to 0.3 for our final LaGarNet model.} This reward function turns out to be easy to learn due to its continuous and linear nature.
...
\subfloat[\rev{Longsleeved T-shirt flattening against SoTA baselines.}]{%
    \includegraphics[width=0.61\linewidth]{plots/lagarnet_flattening_heatmap.png}%
    \label{fig:longsleeve-flattening}}\hfill
\subfloat[\rev{All-garment generalisation by action step 20.}]{%
    \includegraphics[width=0.38\linewidth]{plots/multi_arena_radar_2x2.png}%
    \label{fig:sim-lagarnet-results}}
\caption{\rev{Simulation results for single-arm pick-and-place (\text{PnP}) controllers. \textbf{(a)} Per-horizon longsleeve flattening: rows are methods. Each horizon $N \in \{5, 10, 20, 30\}$ contributes two adjacent columns: \emph{Max}, the best value reached within the first $N$ actions (for SR, the fraction of trials that succeed at any step up to $N$), and \emph{Last}, the value at step $N$ itself. Cell colour encodes the score in $[0,100]$. Only the single-arm PnP primitive of \text{ClothFunnels}~\cite{canberk2023cloth} is enabled; \text{Diffusion Policy}~\cite{kadi2025draper, chi2023diffusionpolicy} is trained on 200 human demonstrations, and \text{MEDOR}~\cite{huang2022mesh} is evaluated on cropped observations (Section \ref{sec:baseline}). NC and Max IoU barely change between Max and Last, whereas SR drops sharply (Section \ref{sec:discussion}). \textbf{(b)} Performance on all four garment types by action step 20, where \text{LaGarNet} consistently outperforms all-garment \text{PlaNet-ClothPick} and task-specific \text{Diffusion Policy}.}}
...
\rev{Figure \ref{fig:longsleeve-flattening} summarises flattening performance across all baselines. All-Garment LaGarNet is comparable to the state-of-the-art mesh-based planner MEDOR and competitive with the human baseline at intermediate horizons, although human experts retain the highest success rate at step 30, because they align the garment more precisely with its flattened shape. Trained on an equivalent volume of data, All-Garment LaGarNet consistently outperforms the Task-Specific LaGarNet trained on longsleeve data alone, showing that the architecture benefits from morphological diversity (Figure \ref{fig:sim-lagarnet-results}; Section \ref{sec:discussion}). Interestingly, given a long enough horizon (30 steps) the task-specific Diffusion Policy---despite only 200 human demonstrations and weak short-horizon behaviour---recovers to highly competitive results. Figure \ref{fig:sim-lagarnet-results} shows that the single-policy \text{LaGarNet} generalises, exceeding 95\% NC on every garment type at action step 20, though higher success rates on skirts and dresses indicate a bias towards simpler topologies.
...
\rev{Figure \ref{fig:horizon-ablation} shows the greedy one-step planner is strongest: success falls monotonically from 73.3\% at $H=1$ to 63.3\%, 53.3\% and 30.0\% at $H=2,3,5$, and the final-step success rate falls with it. A single primitive causes one large deformation, so prediction error compounds along multi-step rollouts and lookahead costs more than it buys, unlike in continuous control \cite{hafner2019learning}. Longer horizons also cost more: mean planning time per action rises from 4.17\,s at $H=1$ to 5.33\,s at $H=5$. The action constraint matters far more: dropping mask rejection at $H=1$ collapses success to near zero and final IoU to 51.0, as most samples miss the garment.}
...
\begin{figure*}[t!]
    ...
    \caption{\rev{Analysis of LaGarNet's real-world performance. \textbf{(a)} Failure cases; text colour indicates the source of failure: white for mechanical limitations, blue for assumption oversights, red for the learning process. \textbf{(b)} Failure-mode frequencies. A step-wise failure does not necessarily cause a trajectory failure; a trajectory succeeds if it reaches the success state within 20 steps. ``Messing-near-success state'' counts how often performance drops in the step immediately after a ``near-success'' step (NC $\geq$ 0.8 and IoU $\geq$ 0.7), and ``As-if-manipulating-other-object'' counts steps where the policy treats part of the garment as though it belonged to a different topology, most often grasping a trouser leg as if it were a sleeve. \textbf{(c)} Timing breakdown across the perception-action cycle. \textbf{(d)} Zero-shot generalisation to an unseen longsleeved dress with soft material properties and high internal friction: the policy still recovers most of the garment's area (77.2 NC), but never reaches the strict joint NC/IoU success criterion.}}
    \label{fig:real_world_analysis}
\end{figure*}

\subsubsection{Results}
\rev{Table \ref{tab:real-worldflattening} shows that the unified all-garment LaGarNet flattens longsleeved T-shirts, trousers, skirts and dresses in the real world, substantially improving on PlaNet-ClothPick, although it struggles with OOD garments (Table \ref{tab:unseen-dress}) and still trails the human baseline, particularly on garments with narrow appendages such as longsleeved T-shirts and trousers. Averaged over all four garments, its NC and NI nonetheless exceed the values reported for single-gripper real-world garment flattening by the mesh-based planners VCD \cite{lin2022learning} and MEDOR \cite{huang2022mesh}, which each report on a single garment type. The results also show a strong topology dependence: LaGarNet reaches a perfect 10/10 success rate on dresses and skirts within 20 steps, whereas the more complex dynamics of trousers and longsleeved shirts yield only 3/10 and 4/10.}
