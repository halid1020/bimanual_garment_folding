...
LaGarNet outperforms strong baselines such as PlaNet-ClothPick \cite{kadi2024planet} and Diffusion Policy \cite{chi2023diffusionpolicy} and matches the mesh-based planner MEDOR \cite{huang2022mesh} with 17$\times$ faster inference and 5$\times$ fewer parameters, planning in a compact latent space rather than over a mesh. It is also competitive with human operators under the same single-arm constraint, although humans succeed more often at long horizons. \irev{In a zero-shot transfer to the real world}, it averages 90\% NC, 80\% NI and 81\% Max IoU on the real garments of Figure \ref{fig:real-garments}. Our contributions are:
...
\end{aligned}
\end{equation}
where $\bm{x}$ represents the observation, $\bm{a}$ represents the action, $\bm{h}$ represents the deterministic latent representation, and ($\hat{\bm{z}}$, $\tilde{\bm{z}}$) represent the posterior and prior stochastic latent states. \rev{In LaGarNet, $\bm{h}$ is 300-dimensional and $\bm{z}$ 60-dimensional.}
...
\rev{As shown in Figure \ref{fig:network}, LaGarNet comprises four components: a convolutional image encoder, a transposed-convolutional decoder, a goal-conditioned transition model built from MLPs and a Gated Recurrent Unit \cite{cho2014properties}, and a 5-layer MLP reward predictor, with ReLU activations throughout. The training objective maximises the marginal log-likelihood of the estimated features and rewards given the action sequence and the target goal observation: $\argmax_{\theta} \ln \irev{p_{\theta}}(\bm{y}_{1:T}, r_{1:T} | \bm{a}_{1:T}, \bm{g})$.}

\rev{Treating this as variational inference and optimising the Evidence Lower Bound (ELBO), the generative joint distribution and approximate posterior read:}
\begin{align}
...
\end{equation}
\rev{where $\mathcal{R}_{dC} = \text{NC}_t - \text{NC}_{t-1}$ and $\irev{\mathcal{R}_{dU}} = \text{MaxIoU}_t - \text{MaxIoU}_{t-1}$; both $\mathcal{R}_{dC}$ and $\irev{\mathcal{R}_{dU}} \in [-1, 1]$, so that $\mathcal{R}_{CA} \in [-1, 1]$. In our experiments, we set $\alpha_r$ to 0.3 for our final LaGarNet model.} This reward function turns out to be easy to learn due to its continuous and linear nature.
...
\rev{We study whether a recurrent latent world model plans effectively in the quasi-static regime, rather than whether its policy holds a flattened state once it reaches one. Nonetheless, we report both last-step and cumulative values over the horizon. Simulated episodes run 30 steps, showing how each policy behaves once the easy gains are exhausted. Real-world trials stop at 20 steps to reduce evaluation time (Section \ref{sec:real-exp})---a human operator comfortably flattens every garment type well within 20 actions, \irev{which is already sufficient to show the difference between LaGarNet and the baselines}.} In both settings each garment's goal state (its maximally flattened configuration) is captured beforehand, serving as the ground-truth reference and as input to goal-conditioned policies: in simulation we use the states of Canberk et al.~\cite{canberk2023cloth}, whilst in the real world we first flatten the garment and then manually crumple it. \rev{This goal is canonical rather than trial-specific: one image per garment, captured once and reused across every trial.} We ignore the slight discrepancy caused by the relative position of garment and camera, as it does not noticeably affect training or evaluation \rev{due to the independent augmentation on the goal and current images during training}.
...
\begin{table*}[t!]
\caption{Real-world garment flattening performance of different pick-and-place controllers. \rev{Each block is an evaluation horizon, and every value is cumulative over that horizon rather than the value at the final step: NC, NI and Max IoU give the best reached within the first 10 or 20 actions, and SR the fraction of trials meeting both criteria at any step up to that point. We report no final-step column here. Each cell already averages only ten physical trials, so a single final-step mean would be noisy, and the failure analysis of Figure \ref{tab:failure-rates} measures end-state behaviour more directly: four of 40 trajectories reach a success state without holding it. } LaGarNet shows substantial performance improvements across all garments compared to PlaNet-ClothPick, though it still falls short of human-level performance.}
...
\rev{We employ a UR5e arm with an OnRobot RG2 parallel gripper, whose usable ring-shaped workspace runs from a near radius $r_n = 0.2$\,m to a far radius $r_f = 0.7$\,m. A RealSense D435i camera at $P_c = (0.03, -0.86, 1.66)$\,m, with a $69^\circ \times 42^\circ$ field of view, observes a dark table surface from the front of the robot; we crop its $1280 \times 720$ frames to a $450 \times 710$ region of interest starting at pixel $(700, 10)$. All physical experiments run on a laptop with Ubuntu 20.04 LTS, an Intel Core i7-12700H, 16\,GB RAM and an NVIDIA RTX 3080 (16\,GB VRAM). Given the sim-to-real gap we relax the success criteria to NC $\geq$ 85 and Max IoU $\geq$ 75. We also analyse computational time consumption and provide a step-wise and trajectory-level failure analysis.}
