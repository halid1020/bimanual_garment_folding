...
\title{LaGarNet: Human-Level Pick-and-Place Garment Flattening with Goal-Conditioned Recurrent State-Space Models}
...
In this paper, we show that our method \text{LaGarNet} can flatten four different types of complex garments, utilising the whole workspace of a robot arm in the real world (Figure \ref{fig:ur3e-real-action-inference}). \text{LaGarNet} outperforms strong learning-based baselines such as PlaNet-ClothPick \cite{kadi2024planet} and Diffusion Policy \cite{chi2023diffusionpolicy}.  Moreover, it  matches the primary performance of mesh-based planning method, MEDOR \cite{huang2022mesh}, but with 17$\times$ faster inference time and 5$\times$ less network parameters. Most importantly, it slightly outperforms human-level performance within 10 steps in simulation. Trained with simulated data and transferred to the real world in the zero-shot manner, LaGarNet reaches on average 90\% NC, 80\% NI and 81\% Max IoU in our real-world experiments on examined garments displayed in Figure \ref{fig:real-garments}. In a nutshell, we make the following four contributions:

1. We propose a goal-conditioned recurrent state-space model (GC-RSSM) that enables a single goal-conditioned garment-flattening planning method \text{LaGarNet} to flatten garments in both simulation and real-world environments.

2. We develop a general data-collection strategy for offline training of \text{PnP} garment-flattening controllers, which is a mixture of an expert \text{Diffusion Policy} learned from few human demonstrations and a masked-biased random policy.
...
% \begin{figure}[t!]
%     \centering
%     \subfloat[\centering Human Policy Trajectory \label{fig:human-tshirt-demo}] {{\includegraphics[width=0.27\textwidth]{plots/huam-trj-with-goal.jpg} }}
%     \subfloat[\centering Rewards \label{fig:reward-diff}] {{\includegraphics[width=0.22\textwidth]{plots/human-rewards.png} }}
...
Our \text{LaGarNet} method for garment flattening is an offline model-based deep reinforcement learning approach. We train LaGarNet with simulation data using domain randomisation (see Figure \ref{fig:sim-train-init}). We transfer it to the real world for evaluation on the real garments presented in Figure \ref{fig:real-garments} in a zero-shot setting. We achieve generalisation on garment flattening through training a goal-conditioned recurrent state-space model (see Section \ref{sec:gc-rssm} and \ref{sec:architecture}) on a coverage-alignment reward function designed for all garment-flattening tasks (see Section \ref{sec:ca-reward}). We collect simulation data with a more unified data collection procedure (Section \ref{sec:data-collection}) that uses task-specific Diffusion Policies (see Section \ref{sec:diffusion}) as a proximal expert policy to collect trajectories around the successful distribution. Then, we use a set of data augmentation techniques (see Section \ref{sec:data-aug}) during the training to enable our all-garment LaGarNet to achieve state-of-the-art flattening performance with only 100 thousand interaction steps.

Our design takes inspiration from the working-memory literature in cognitive psychology: humans maintain and update task-relevant internal representations to infer hidden states from incomplete observations, predict future outcomes, and plan towards a desired state \cite{miller2001integrative}. LaGarNet mirrors this process with a goal-conditioned recurrent state-space model that maintains a belief state over the partially observed garment configuration, together with a reward predictor that evaluates how candidate actions advance the task towards its goal \cite{hafner2019learning}.


\subsection{Goal-Conditioned Recurrent State Space Model} \label{sec:gc-rssm}


Our method LaGarNet builds on the mathematical model of State-Space Models (SSMs) that primarily function as generative frameworks for sequential data, supporting tasks such as inferring future states and learning representations \cite{ma2024interpretable}. Hafner et al. \cite{hafner2019learning} define the Recurrent State-Space Model (RSSM), using a recurrent neural network, under the Partially Observable Markov Decision Process (POMDP) formulation with the following latent state dynamics:
\begin{equation}
\begin{aligned}
...
During training, we do not stop any gradients on the goals, but we stop them for overshooting loss. We use GC-RSSM to avoid recurrent structure of the model from ``forgetting'' information of the goal. Combining all the aforementioned components, we formally express the total loss function as:
\begin{equation}
...
\subsubsection{Reward Predictor} The reward model $r_\theta(\bm{h}_t, \bm{z}_t)$ predicts the expected scalar reward $r_t$ from the current latent state. It takes the concatenated deterministic state $\bm{h}_t$ and stochastic state $\bm{z}_t$, processes them through four hidden layers and projects them to a scalar value via a final linear layer to formulate a 5-layer MLP.

\subsection{Coverage-Alignment Reward}  \label{sec:ca-reward}

Our coverage-alignment reward $\mathcal{R}_{CA}$  uses the step-wise normalised coverage (NC) difference $\mathcal{R}_{dC}$ and the difference of maximum intersection-over-union (\text{Max IoU}) between the current and goal cloth mask, denoted as $\mathcal{R}_{du}$. Mathematically, our reward function can be expressed as follows: 

\begin{equation}
    \mathcal{R}_{CA} = \alpha_r \times\mathcal{R}_{dc} + (1-\alpha_r)\times \mathcal{R}_{du},
\end{equation}
where $\mathcal{R}_{dC} = \text{NC}_t - \text{NC}_{t-1}$ and $\mathcal{R}_{du} = \text{MaxIoU}_t - \text{MaxIoU}_{t-1}$; both $\mathcal{R}_{dC}$ and $\mathcal{R}_{du} \in [-1, 1]$, so that $\mathcal{R}_{CA} \in [-1, 1]$. In our experiments, we set $\alpha_r$ as 0.3 for our final LaGarNet. This reward function turns out to be easy to learn due to its continuous and linear nature.
...
1) \textbf{Section \ref{sec:comp-sota}:} How does LaGarNet's performance compare to other state-of-the-art (SoTA) baselines for single-picker pick-and-place flattening methods?

2) \textbf{Section \ref{sec:comp-diff}:} What is the advantage of using the GC-RSSM model in the garment flattening domain compared to a diffusion policy? 

3) \textbf{Section \ref{sec:abl}:} How important are the different components of LaGarNet? 

4) \textbf{Section \ref{sec:real-exp}:} How does LaGarNet perform in the real world?    


To rigorously evaluate the performance of our garment flattening policy, we employ the following standard metrics:

1) \textbf{Normalised Coverage (NC):} The ratio of the garment's current 2D projected area to its maximally flattened area. This metric isolates the extent of unfolding and is bounded to $[0, 1]$.

2) \textbf{Normalised Improvement (NI):} The relative increase in coverage from the initial configuration, defined as $\frac{c_t - c_0}{c_{\text{max}} - c_0}$, where $c_t$, $c_0$, and $c_{\text{max}}$ denote the current, initial, and maximally flattened coverage, respectively. 

3) \textbf{Maximum Intersection-over-Union (Max IoU):} The highest IoU overlap between the current garment segmentation mask and the target flattened mask. This evaluates the precise spatial alignment and shape matching of the garment through rotation and translation.

4) \textbf{Success Rate (SR):} The percentage of trials that reach a state satisfying strict flattening and alignment criteria, empirically defined as achieving both $\text{NC} > 90\%$ and $\text{Max IoU} > 80\%$ in the same state.

All reported metrics are cumulative over the execution horizon: for NC, NI, and Max IoU, the value reported at a horizon of $k$ steps is the maximum achieved within the first $k$ action steps of a trajectory, rather than the instantaneous value at step $k$; similarly, the SR at $k$ steps counts the trials that have satisfied the success criteria at any step up to $k$. Unless stated otherwise, we report all metrics over a 20-step horizon. For both simulated and real-world experiments, the goal state (i.e., the maximally flattened configuration) of each garment must be predefined and captured prior to the execution of the trials to serve as the ground-truth reference for computing the aforementioned metrics---it is also needed as an input to goal-condition policies. In simulation, we use the off-the-shelf generated states by Canberk et al. \cite{canberk2023cloth}, whilst in the real world, we must first set the garment to flattened states then manually put it into crumpled states to start. In both simulation and real-world environment, we ignore the slight discrepancy caused by the relative position between the garment and camera viewpoint and its field of view, as it does not obviously affect either the training or evaluation of these policies in both setups.
\begin{table}[!t]
\centering
\caption{Simulation Configurations. We need to scale down the garments 0.32 on our longsleeve set so to get the optimal performance for MEDOR.}
...
\begin{figure*}[t!]
    ...
    \end{subfigure}
    \caption{The initial state distribution of simulation garments.}
...
\begin{figure*}[t!]
\centering
\includegraphics[width=\textwidth]{plots/lagarnet_flattening_heatmap.png}
\caption{Longsleeved T-shirt flattening in simulation: per-horizon performance of learning-based single-arm pick-and-place (\text{PnP}) controllers. For each metric (Normalised Coverage, Normalised Improvement, Max IoU, and Success Rate), the columns report the cumulative \emph{Max} (best value reached within the horizon) and the \emph{Last} (final-step) value at 5, 10, 20, and 30 action steps; cell colour encodes the score in $[0,1]$. We enable only the single-arm PnP primitive of the off-the-shelf \text{ClothFunnels}~\cite{canberk2023cloth} to keep a consistent environment; \text{PlaNet-ClothPick} and \text{LaGarNet} use the RGB-to-Mask setup on the same task-focused and all-garment datasets with their own reward functions, while \text{Diffusion Policy}~\cite{kadi2025draper, chi2023diffusionpolicy} is trained on 200 human demonstrations. The state-of-the-art mesh-based planner \text{MEDOR}~\cite{huang2022mesh} is evaluated with its own simulation setup. Our All-Garments \text{LaGarNet} reaches performance comparable to MEDOR and closely matches human experts (and slightly exceeds them at 10--20 steps); the small Max$\to$Last gap indicates that final configurations remain stable.}
\label{fig:longsleeve-flattening}
\end{figure*}



This section compares various state-of-the-art (SoTA) methods for single-arm pick-and-place (PnP) in simulation. We first introduce our simulation setup (Section \ref{sec:simulation}), followed by our rationale for selecting and evaluating various SoTA baselines (Section \ref{sec:baseline}). Then, we analyse the primary and secondary performance of these methods (Section \ref{sec:sota-result}).

\subsubsection{Simulation Setup} \label{sec:simulation}

We develop \text{LaGarNet} using the garment simulation environment from Canberk et al.~\cite{canberk2023cloth} which is built on SoftGym \cite{lin2020softgym} and PyFlex physics engine \cite{li2018learning}. Following the real-world adaptation strategy of the \text{DRAPER} methodology \cite{kadi2025draper}, we extend the simulation to include misgrasping and multi-layer grasping scenarios, inherent to simple parallel grippers. We focus on simulated longsleeved T-shirts, trousers, dresses, and skirt objects selected by Canberk et al. from the commonly employed \text{Cloth3D} dataset \cite{bertiche2020cloth3d} (see Figure \ref{fig:sim-train-init}). Please see the Table \ref{tab:sim-config} for our simulation configuration. Following Canberk et al., we scale the garments by a factor of 0.8 and set the a simulated top-down camera height with a completely dark background for vision rendering. They split the generated initial garments into ``hard'' and ``easy'' categories, where the former is more crumpled and the latter is more flattened; we use 30 ``hard'' initial states placed within the defined workspace of the gripper for evaluation. Please see Figure \ref{fig:garment-distribution} for the distribution of our simulation garments.
...
All trained methods apart from \text{ClothFunnels} and \text{MEDOR} employ the vision processing strategy from the \text{DRAPER} framework to achieve simulation-to-reality transfer \cite{kadi2024mjtn}; this strategy attempts to align  real and synthetic RGB observations, applying real-world adaptation and domain randomisation in simulation, as well as a refined grasping procedure to reduce failures. While we train \text{Diffusion Policy} on 200 human expert trajectories, we let \text{LaGarNet} and \text{PlaNet-ClothPick} learn from 5 thousand episodes (i.e., 100 thousand transitional steps) generated from our data-collection strategy. \text{PlaNet-ClothPick} uses similar set of hyperparameters as \text{LaGarNet} but with different reward function and without goal observations. The all-garment  variants of these methods adopts the trajectories from all four garment types, whilst keeping the same data size as task-specific datasets. We conduct human experiments through a PnP window interface operating directly on observation images both in simulation and the real world.

\begin{figure*}[t!]
    \centering
    \includegraphics[width=1.0\linewidth]{plots/multi_arena_radar.png}
    \caption{Simulation performance of different policies on all four garment types by action step 20. Our method \text{LaGarNet} consistently outperforms the baseline all-garment \text{PlaNet-ClothPick} and task-specific \text{Diffusion Policy} on each garment type.}
    \label{fig:sim-lagarnet-results}
\end{figure*}

\begin{table*}[t!]
    \centering
    \caption{Secondary comparison of computational requirements and data efficiency across garment flattening baselines. $\dagger$ represents the reported values in the original paper. \text{ClothFunnels} uses a machine with 4 NVIDIA RTX3090s. Our hardware setup for all timing and parameter evaluations is: Ubuntu 24.04.4 LTS, NVIDIA GeForce RTX 3090 Ti GPU, AMD Ryzen 9 5900X 12-Core Processor (2 threads per core), and 64GB RAM. Whist mesh-based method requires substantially shorter data collection time, our LaGarNet method demands much less network parameters and faster training and per-step inference time.}
...
Please refer to Section \ref{sec:real-world-setup} for details regarding the real-world robot setup. We conduct all physical experiments on a laptop running Ubuntu 20.04 LTS, equipped with an Intel Core i7-12700H processor, 16 GB of system RAM, and an NVIDIA GeForce RTX 3080 GPU with 16 GB of VRAM. Given the inherent sim-to-real challenges of deploying a simulation-trained policy in the physical world, we relax the success criteria to Normalised Coverage (NC) $\geq$ 85 and Max IoU $\geq$ 75. Moreover, whilst our simulation study evaluates policies for up to 30 action steps, we cap each real-world trial at 20 steps: each perception-action cycle takes approximately 25 seconds (see Table \ref{tab:timing_metrics}) in addition to manual garment resets between trials, so extending every trial across all policies and garment types to 30 steps would add hours of robot experimentation time, whilst most trials plateau before step 20 (see Table \ref{tab:real-worldflattening}). Beyond these primary metrics, we analyse the system's computational time consumption and provide a comprehensive step-wise and trajectory-level failure analysis.
...
\begin{figure*}[t!]
    ...
    \begin{subfigure}[b]{0.45\linewidth}
        ...
        \caption{Heuristic of Transferring Policy in Region-of-Interest (ROI)}
