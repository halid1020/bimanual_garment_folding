...
\begin{table*}[t!]
\caption{Numerical comparison of different learning-based pick-and-place (\text{PnP}) controllers on longsleeved T-shirts in simulation. We only enable the single-arm PnP primitive of the off-the-shelf \text{ClothFunnels} \cite{canberk2023cloth} to ensure that all models operate in a consistent environment. We train both \text{PlaNet-ClothPick} and \text{LaGarNet} with the RGB-to-Mask setup on the same task-focused and all-garments datasets, yet with their own reward functions and network architectures. We train the behaviour cloning methods \text{Diffusion Policy} \cite{kadi2025draper, chi2023diffusionpolicy} with 200 human demonstrations. Each horizontal block reports cumulative metrics within the first 5, 10, 20, and 30 action steps, respectively. Mesh-based planning method, $\dagger$ VCD \cite{lin2022learning}, reports to reach 83.8 $\pm$ 45 \% NI and 96 $\pm$ 21.8 \% NC for T-shirt flattening in simulation within 20 steps; however, its improved successor, MEDOR \cite{huang2022mesh}, stably outperforms it in all scenarios. We test the off-the-shelf MEDOR in our evaluation trials but with their simulation setup. We highlight the top three values for each metric and action  step with green, yellow, and red backgrounds, respectively. The proposed All-Garments \text{LaGarNet} outperforms all compared algorithms; it reaches a similar performance to the SoTA mesh-based method MEDOR, and even slightly outperforms human experts from 10-20 steps.}
