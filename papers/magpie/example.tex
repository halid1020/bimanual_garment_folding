...
\usepackage{stfloats} % allow bottom-placed figure* on the page where it is issued (page-1 teaser)
...
\begin{figure*}[!b]
    \centering
    \includegraphics[width=\linewidth]{plots/Magpie-Primitives.jpg}
    \caption{MAGPIE's action primitives executed on our physical dual-arm system.
    From a crumpled initial state (left), the policy sequences three parameterised
    primitives---single-arm pick-and-place, dual-arm pick-and-fling, and dual-arm
    pick-and-place---to flatten, align, and fold the garment.}
...
MAGPIE adopts four types of primitives essential for effective and robust performance; see Figure \ref{fig:primitives}. Each primitive is parameterised by a set of normalised 2-D pixel coordinates in $[-1, 1]$ (a total of $4$, $8$, $4$, and $0$ parameters, respectively), which are de-normalised and projected onto the table plane before execution.
...
\subfloat[Data sampling and augmentation.]{%
    \raisebox{\dimexpr-\height}{\includegraphics[width=0.61\linewidth, trim=0 0 2088bp 0, clip]{plots/dataloader_augment_master_multi_batch.png}}%
...
This section first introduces the implementation details of behaviour primitives in the real world (Section \ref{sec:real-world-primitives}). Next, we discuss how we conduct our hand-eye calibration (Section \ref{sec:calibration}) for the transformation between the camera pixel space and robot base space. Then, we explain our approach to segmenting the physical garments from the camera observations (Section \ref{sec:real-world-cloth-seg}), and detail our grasping implementation, designed to ensure the precise and robust picking of garments (Section \ref{sec:grasping}). Finally, we describe the real-world goal-directed folding environment, the physical counterpart of the simulated goal-directed folding task (Section \ref{sec:real-world-folding-env}).
...
\paragraph{Dual-Arm Pick-and-Place.} The dual-arm PnP primitive handles simultaneous spatial manipulation, taking two pick-and-place pairs as input. To resolve arm assignments, the pick points are sorted spatially from left to right along the X-axis. Before execution, the system mathematically approximates the paths of both robot arms and computes the minimum distance between their trajectories. If the paths come within a strict 0.15~m threshold, the system aborts the simultaneous execution and degrades to sequential single-arm PnP to prevent hardware collision. During valid dual-arm executions, both robots approach their targets and conduct force-guarded grasping simultaneously. To transfer the garment smoothly, we utilise an \textit{arc} trajectory mode. Instead of a rigid rectangular path that leads to not-desirable folding, the system generates a discrete 6-waypoint parabolic path. The apex height of the parabola is dynamically scaled to 30\% of the transit distance (capped at 20~cm), and the robot controller blends these waypoints with a 0.02~m radius to maintain continuous momentum. Upon reaching the place location, a dedicated \textit{tension release} routine is invoked prior to opening the grippers, since a garment stretched between the two grippers otherwise snaps back from the place points upon release. Both robots switch into force-compliance mode in the horizontal plane and each applies a 4~N wrench along its \emph{true} inward direction---the unit vector towards the other arm's end-effector, computed per arm in its own base frame via the calibrated inter-base transform. The routine terminates on force feedback: the wrist force--torque sensors are polled at 10~Hz, and the release ends once both arms report slack fabric (horizontal tension below 1.5~N or a ${\geq}60\%$ drop from its initial value), or otherwise on a 15~cm closure limit, an end-effector settle velocity below 0.005~m/s, or a 3~s cap. If either end-effector sits within 15~cm of a robot base (the shoulder-singularity zone), or if force mode triggers a protective stop, the system falls back to an equivalent position-controlled converge that steps both grippers inward by $\min(4~\text{cm},\, 15\%$ of the gripper separation$)$. Finally, before opening, both grippers perform a slow \textit{deposit} descent from the 2~cm transit height to 5~mm above the place height, laying the slackened fabric onto the table so that it retains the commanded placement.
...
\subsection{Real-World Goal-Directed Folding Environment} \label{sec:real-world-folding-env}

Mirroring the simulated goal-directed folding task (Section \ref{app:eval_protocol}), the real-world environment requires a garment initialised in a random crumpled state to pass \emph{through} a canonical flattened pose before being folded, i.e.\ an ordered set of $K=3$ subgoals: the canonical/aligned flattened state $S_0$ followed by two fold states $S_1, S_2$. Goals are specified once per garment instance and cached. First, the operator defines $S_0$ through an interactive interface that overlays the segmented flattened garment mask on the two arms' reachable workspace regions together with a horizontal mid-line, and is instructed to translate and rotate the mask so that the garment's hem faces the top of the image and the shoulder line coincides with the image's horizontal mid-line---placing the canonical pose at the intersection of the two workspaces, as in simulation. The operator then physically arranges the flattened garment to match this target under live IoU feedback, and demonstrates the two fold steps using the behaviour primitives; the captured post-fold observations become $S_1$ and $S_2$.

During evaluation, the environment computes the strict Alignment IoU of the current garment mask against every subgoal and feeds the policy (or the human interface) the \emph{active} subgoal: the one following the furthest subgoal already achieved as a contiguous temporal suffix of the trajectory, so that a failed fold correctly reverts the target to the canonical state. An episode is successful---the real-world analogue of the Sequential IoU-Based Success Rate (Sq-IoU-SR)---when the trajectory reproduces $S_0, S_1, S_2$ in order, each above its per-subgoal IoU threshold of $(0.7, 0.7, 0.7)$, matching the simulated task; the criterion is re-verified against the final subgoal at every subsequent step. Alongside the per-step observations, the environment records the terminal state reached after the final action for post-hoc evaluation.
