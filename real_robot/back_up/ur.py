...
def main():
    ...
    # Initialize robot (gripper=None or 'rg2' if you have it)
    ur5 = UR5RTDE(robot_ip, gripper='rg2')

    try:
        print("Connecting to UR5 robot...")

        # Move to home position
        print("Moving to home position...")
        ur5.home(speed=1.0, acceleration=0.8, blocking=True)
        ...
        # Define a target pose slightly above current position
        target_pose = tcp_pose.copy()
        target_pose[2] += 0.05  # move 5 cm up

        print("Moving linearly to target pose...")
        ur5.movel(target_pose, speed=0.25, acceleration=0.5, blocking=True)
        ...
        # Get TCP speed and force
        print(f"TCP Speed: {ur5.get_tcp_speed()}")
        print(f"TCP Force: {ur5.get_tcp_force()}")
