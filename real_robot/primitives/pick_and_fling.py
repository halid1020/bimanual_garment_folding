...
class PickAndFlingSkill:
    ...
    def dual_arm_stretch_and_fling(self, 
            ur5e_pick_point_ur5ebase, 
            ur16e_pick_point_ur5ebase,
            stretch_force=20,
        ...
        # --- DEBUG: Initial State ---
        initial_width = self.scene.get_tcp_distance()
        print("\n" + "="*60)
        print(f"[FLING DEBUG] Phase 1: Pre-Stretch")
        print(f"  Initial Width:      {initial_width:.4f} m")
        print("="*60)

        ur5e_pose_world, ur16e_pose_world = points_to_gripper_pose(
            ur5e_pick_point_ur5ebase, ur16e_pick_point_ur5ebase, max_width=stretch_max_width)
        ...
        # --- DEBUG: Post-Stretch State ---
        final_width = self.scene.get_tcp_distance()
        stretch_delta = final_width - initial_width
        
        print("\n" + "="*60)
        print(f"[FLING DEBUG] Phase 2: Post-Stretch Analysis")
        print(f"  Final Width:        {final_width:.4f} m")
        print(f"  Width Change:       {stretch_delta:+.4f} m")

        # Get current TCP poses in their respective local base frames
        current_p5_base = self.scene.ur5e.get_tcp_pose()   # UR5e Base Frame
        current_p16_base = self.scene.ur16e.get_tcp_pose() # UR16e Base Frame
        ...
        ur5e_path_full_ur5ebase, ur16e_path_full_ur5ebase = points_to_fling_path(
            left_point=new_ur5e_point,
            right_point=new_ur16e_point,
            width=final_width, 
        ...
        # --- FIX START: Overwrite Start Points ---
        # The path generator creates a theoretical start point [0] based on perfect symmetry.
        # We overwrite the position (XYZ) of the first waypoint [0] with the ACTUAL current robot position.
        # This prevents any "adjustment" or "widening" move at the start of the trajectory.
        # Note: We keep the rotation generated by the path planner to ensure the fling rotation is consistent,
        # but you could overwrite that too if you wanted to maintain grasp angle strictly.
        
        print(f"  [FIX] Overwriting Path Start [0] to match Actual Position exactly.")
        print(f"    UR5e Path[0] Before:  {np.round(ur5e_path_full_ur5ebase[0][:3], 4)}")
        ur5e_path_full_ur5ebase[0][:3] = new_ur5e_point
        print(f"    UR5e Path[0] After:   {np.round(ur5e_path_full_ur5ebase[0][:3], 4)}")
        
        print(f"    UR16e Path[0] Before: {np.round(ur16e_path_full_ur5ebase[0][:3], 4)}")
        ur16e_path_full_ur5ebase[0][:3] = new_ur16e_point
        print(f"    UR16e Path[0] After:  {np.round(ur16e_path_full_ur5ebase[0][:3], 4)}")
        # --- FIX END ---

        # 3. Slice the paths
        ur5e_fling_path = ur5e_path_full_ur5ebase[:-1]
        ur16e_fling_path = ur16e_path_full_ur5ebase[:-1]
        
        ur5e_drag_target = ur5e_path_full_ur5ebase[-1]
        ur16e_drag_target = ur16e_path_full_ur5ebase[-1]
        ...
        # --- DEBUG: Execution Transformation ---
        print("\n" + "="*60)
        print(f"[FLING DEBUG] Phase 4: Execution Transformation (Target: UR16e Base Frame)")
        print(f"  Transformed UR16e Path for Execution:")
        print(f"    Start Point (in UR16e Base): {np.round(ur16e_fling_path_base[0][:3], 4)}")
        print(f"    End Point   (in UR16e Base): {np.round(ur16e_fling_path_base[-1][:3], 4)}")
        print("="*60 + "\n")
